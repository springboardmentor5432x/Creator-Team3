...
export default function PlatformDashboardView({ platformKey, token, setActiveTab }) {
  ...
  const fetchPlatformData = async (handleQuery = '') => {
    try {
      ...
      }else if (key === "youtube" && handleQuery) {
        url = `http://127.0.0.1:8000/api/social/youtube/${encodeURIComponent(handleQuery)}`;
      } else {
          url =
                `http://127.0.0.1:8000/api/analytics/platform/${key}` +
