...
import FollowerGrowthAnalytics from './FollowerGrowthAnalytics';
import ReachImpressionsAnalytics from './ReachImpressionsAnalytics';
import EngagementTrendsAnalytics from './EngagementTrendsAnalytics';
...
export default function AudienceAnalyticsView({ token }) {
  ...
  const fallbackData = {
    ...
    engagementTrends: [
      ...
      { date: "2026-07-29", likes: 68000, comments: 4000, shares: 1800 }
    ],
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', overflowX: 'auto' }}>
          {['overall', 'youtube', 'instagram', 'twitter', 'linkedin'].map(p => (
            <button
              key={p}
      ...
      <div className="theme-card" style={{ padding: '22px' }}>
        ...
        <div style={{ marginTop: '32px' }}>
          <h4 style={{ margin: '0 0 16px', fontSize: '14px', fontWeight: 600, color: 'var(--text-primary)' }}>Most Active Days</h4>
          <div style={{ height: '180px', width: '100%' }}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={displayData.activeDays}>
                <CartesianGrid strokeDasharray="3 3" stroke="var(--border-primary)" vertical={false} />
                <XAxis dataKey="day" stroke="var(--text-secondary)" style={{ fontSize: '11px' }} />
                <YAxis stroke="var(--text-secondary)" style={{ fontSize: '11px' }} />
                <Tooltip contentStyle={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border-primary)', color: 'var(--text-primary)', borderRadius: '8px' }} cursor={{ fill: 'var(--bg-input)' }} />
                <Bar dataKey="activity" fill="#10b981" radius={[4, 4, 0, 0]} name="Activity Level" />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      <FollowerGrowthAnalytics data={displayData.followerGrowth} />
      <ReachImpressionsAnalytics data={displayData.reachImpressionTrends} />
      <EngagementTrendsAnalytics data={displayData.engagementTrends} />
