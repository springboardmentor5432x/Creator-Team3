...
def get_platform_dashboard(platform: str, user=Depends(verify_token), db: Session = Depends(get_db)):
    db_user = get_or_create_user_from_token(user, db)
    ...
    if p_clean == "instagram":
        ...
        if social_acc:
            yt = YouTubeService()
            return yt.get_channel_details(social_acc.account_name)
        return {"connected": False, "platform": "YouTube", "message": "YouTube Channel not connected."}
