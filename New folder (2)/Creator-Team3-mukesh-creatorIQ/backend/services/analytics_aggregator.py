...
from models import CreatorProfile, SocialAccount, ContentLink, InstagramAccount, Growth
...
class AnalyticsAggregator:
    ...
    def get_aggregated_dashboard_data(cls, user_id: int, db: Session) -> Dict[str, Any]:
        ...
        print("CREATOR ID =", profile.creator_id)
        print("PROFILE =", profile)
        all_accounts = db.query(SocialAccount).all()

        print("USER ID =", user_id)
        print("CREATOR ID =", profile.creator_id if profile else None)
        print("TOTAL SOCIAL ACCOUNTS =", len(all_accounts))
        ...
        print("MATCHED SOCIAL ACCOUNTS =", accounts)
    ...
    acc.account_name
            ...
            if p_name == "YouTube":
                ...
                if yt_data.get("connected"):
                    ...
                    f_count = yt_data.get("subscribers", f_count)
                    p_views = yt_data.get("views", p_views)
                    p_watch_hrs = yt_data.get("watch_time_hours", p_watch_hrs)
                    yt_rev = yt_data.get("estimated_revenue", 0)

            else:
                yt_rev = round((p_views / 1000.0) * 2.80, 2)
        ...
        existing = (
            ...
            .filter(
                ...
                Growth.date >= datetime.combine(today, datetime.min.time()),
                Growth.date < datetime.combine(today + timedelta(days=1), datetime.min.time())
            )
            .first()
        )
